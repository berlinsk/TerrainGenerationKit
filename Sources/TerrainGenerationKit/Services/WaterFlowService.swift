...
public final class WaterFlowService: WaterFlowServiceProtocol, @unchecked Sendable {
    ...
    private let gpuWater: GPUWaterProcessor?

    public init() {
        if let gpu = GPUComputeEngine.shared {
            self.gpuWater = GPUWaterProcessor(gpu: gpu)
        } else {
            self.gpuWater = nil
        }
    }
    ...
    public func generateWaterBodies(
        ...
        if let gpuWater = gpuWater,
           let result = gpuWater.simulateWaterFlow(
               heightmap: heightmap,
               width: width,
               height: height,
               params: params,
    ...
    private func normalizeWaterDepth(waterData: inout WaterData) {
        guard let maxDepth = waterData.waterDepth.max(), maxDepth > 0 else {
            return
        }
